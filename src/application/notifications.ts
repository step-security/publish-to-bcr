...
`;
    ...
    if (!friendlyErrors.length) {
      content +=
        'An unknown error occurred. Please report an issue here: https://github.com/step-security/publish-to-bcr/issues.';
    }

    console.error(`Sending error email to ${recipients.join(', ')}`);
